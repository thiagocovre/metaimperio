        ...
        foreach($commissionRules as $commissionRule) {
            ...
            if(is_array($commissionRule->getGroupId()) && in_array($seller->getGroupId(), $commissionRule->getGroupId())) {
                foreach ($rules as $key => $_rules) {
                    if((in_array($storeId, $_rules->getData('store_id')) || in_array(0, $_rules->getData('store_id'))) && $_rules && count($_rules->getData()) >0 && in_array($seller->getGroupId(), $_rules->getGroupId()) ){
                ...
                if((in_array($storeId, $commissionRule->getData('store_id')) || in_array(0, $commissionRule->getData('store_id'))) && count($commissionRule->getData()) >0 ){
                  
                    $collection->getSelect()->reset(\Magento\Framework\DB\Select::WHERE);         
                    $conditions = $commissionRule->getActions();            
                    $conditions->collectValidatedAttributes($collection);  
                    $collection->getSelect()->where('e.entity_id IN (?) ',$entity_id);
