...
class MessageAdmin extends \Magento\Framework\View\Element\Template {
    ...
    */
    ...
    public function getMessage() {
        $message = $this->message->getCollection()->addFieldToFilter('seller_id',$this->getSellerId());
        return $message;
    }
     /**
     *  get Seller Id
     ...
     */
     ...
     public function getUnreadMessage($message_id){ 
        ...
        $unread = $this->detail->getCollection()->addFieldToFilter('message_id',$message_id)->addFieldToFilter('receiver_id',$this->getSeller()->getSellerId())->addFieldToFilter('is_read',0);
        if(count($unread) > 0) {
            $count = 'Unread';
        } else {
            $count = 'Read';
